...
DIAGRAM, JS components, stores etc, gets fed into gulp which runs webpack and babel compared with equivalent for TypeScript
...
## 4 Import type definitions for external node_modules libraries that we were using
...
## Getting the test suite up and running again
 ...
 Fix up our tests runner (jest) to compile the TypeScript classes to javascript on each test run
...
# Result
Our Bet Tracker product is now fully TypeScripted. We've not added a typehint to every parameter or method but in order to address our initial problem, we've added types to all of our models for bets, selections and outcomes.
